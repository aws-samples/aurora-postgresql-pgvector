...
function install_requirements()
{
    ...
    echo "Shell output of installing requirements for Q&A ${?}"

    echo "Installing python requirements"
    cd $HOME/environment/${PROJ_NAME}/02_RetrievalAugmentedGeneration/02_ResponseStreaming
    pip3.11 install -r requirements.txt > ${TERM} 2>&1
    echo "Shell output of installing requirements for Response Streaming ${?}"
...
function cp_logfile()
{
    ...
    bucket_name="apgpgvector-${AWS_ACCOUNT_ID}-`date +%s`"
    echo ${bucket_name}
